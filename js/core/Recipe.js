...
class Recipe {
    constructor(id, name, resultItem, materials, requiredLevel = 1, requiredSkill = null) {
        ...
        this.requiredSkill = requiredSkill; // Требуемый навык (если есть)
        this.isUnlocked = false; // Открыт ли рецепт
        this.unlockChance = 0.05; // 5% шанс открыть при крафте или в бою
...
class RecipeManager {
    ...
    // Открыть рецепт при получении навыка
    unlockRecipeBySkill(skillId) {
        // Здесь будет логика открытия рецептов при получении навыков
        // Например, навык "Кузнечное дело" открывает рецепты оружия
        console.log(`Попытка открыть рецепты для навыка ${skillId}`);
    }
